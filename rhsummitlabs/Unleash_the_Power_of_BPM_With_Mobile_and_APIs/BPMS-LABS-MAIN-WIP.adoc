...
As discussed BPM has separated its Design time and Runtime operations into 2 distinct binaries; Business Central for Design Time and the Intelligent Process Server (KIE Server) for Runtime.
...
----
curl -X PUT \
  --user <USER_ID>:'<USER_PWD>' http://<HOST>:<PORT>/kie-server/services/rest/server/container/<CON> \
