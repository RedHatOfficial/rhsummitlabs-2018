...
- Firstly set the following environment variable for the BPM hostname
[source, sh]
----
# IF USING BPM SERVER 1
export HOSTNAME = ec2-34-251-130-28.eu-west-1.compute.amazonaws.com:8080

# IF USING BPM SERVER 2
export HOSTNAME = ec2-52-51-30-91.eu-west-1.compute.amazonaws.com:8080
----

- We can check to see if our IPS/KIE Server is alive and available:

REQUEST
[source,sh]
----
curl -X GET --user <USER_ID>:<USER_PWD> http://$HOSTNAME/kie-server/services/rest/server
----
...
<response type="SUCCESS" msg="Kie Server info">
    <kie-server-info>
        ...
        <location>http://$HOSTNAME/kie-server/services/rest/server</location>
        <messages>
            <content>Server KieServerInfo{serverId='15ad5bfa-7532-3eea-940a-abbbbc89f1e8', version='6.5.0.Final-redhat-16', location='http://$HOSTNAME/kie-server/services/rest/server'}started successfully at Tue Apr 03 22:41:15 BST 2018</content>
...
curl -X PUT \
  --user <USER_ID>:<USER_PWD> http://$HOSTNAME/kie-server/services/rest/server/containers/$ALIAS \
...
----
curl -X GET \
  --user <USER_ID>:<USER_PWD> http://$HOSTNAME/kie-server/services/rest/server/containers
----
...
NOTE: The whole KIE REST API can be reviewed at: http://$HOSTNAME/kie-server/docs/index.html
...
curl -X GET \
  http://$HOSTNAME/kie-server/services/rest/server/queries/processes/definitions  \
...
curl -X GET \
  http://$HOSTNAME/kie-server/services/rest/server/queries/containers/<CONTAINER_ID>/processes/definitions  \
  -H 'accept: application/json' \
  -H 'authorization: Basic YnBtc0FkbWluOnBhc3N3b3JkMSE='
----
...
[source,sh]
----
http://$HOSTNAME/kie-server/services/rest/server/containers/<CONTAINER_ID>/images/processes/org.jbpm.demo.NewApplication
----
...
- This demo is part of a larger Financial Onboarding demo, which has been simplified to show a wider array of product functionality. As a side effect, we are required to provide the object model upfront rather than letting the BPM process hit various services. Using this payload we will create a new process instance, which will return a Process Instance Id.
 ...
 . CONTAINER_ID - This is your custom container ID which was exported as the environemnt variable ALIAS
 . PROCESS_DEFINITIONS_ID - org.jbpm.demo.NewApplication
 . Full URL Structure: http://$HOSTNAME/kie-server/services/rest/server/containers/<CONTAINER_ID>/processes/<PROCESS_DEFINITION_ID>/instances

REQUEST:
[source,sh]
----
curl -X POST --user <USER_ID>:<USER_PWD> \
  http://$HOSTNAME/kie-server/services/rest/server/containers/$ALIAS/processes/org.jbpm.demo.NewApplication/instances \
  -H 'cache-control: no-cache' \
  -H 'accept: application/json' \
...
----
 ...
 - Using the REST API, start an instance of the org.jbpm.demo.NewApplication process.
  ...
  .. In a browser, navigate to http://$HOSTNAME/kie-server/services/rest/server/containers/<CONTAINER_ID>/images/processes/instances/1, replacing 1 with the process instance ID of the process instance you just created. +
...
Full URL Structure: http://$HOSTNAME/kie-server/services/rest/server/containers/<CONTAINER_ID>/process/instances?<PARAMS> +

* REQUEST:
[source,sh]
----
curl -X POST --user <USER_ID>:<USER_PWD> \
  'http://$HOSTNAME/kie-server/services/rest/server/queries/containers/<CONTAINER_ID>/process/instances?status=1&page=0&pageSize=25' \
  -H 'accept: application/json' \
  -H 'cache-control: no-cache' \
  -H 'accept: application/json' \
----
...
----
curl -X POST --user <USER_ID>:<USER_PWD> \
  'http://$HOSTNAME/kie-server/services/rest/server/queries/tasks/instances/process/<pInstanceId> ' \
  -H 'accept: application/json' \
  -H 'cache-control: no-cache' \
  -H 'accept: application/json' \
----
...
----
curl -X POST --user <USER_ID>:<USER_PWD> \
  'http://$HOSTNAME/kie-server/services/rest/server/queries/tasks/instances/pot-owners ' \
  -H 'accept: application/json' \
  -H 'cache-control: no-cache' \
  -H 'accept: application/json' \
----
...
[source,sh]
----
 curl -X PUT -H "Accept: application/json" --user <USER_ID>:<USER_PWD> "http://$HOSTNAME/kie-server/services/rest/server/containers/<CONTAINER_ID>/tasks/<TASK_ID>/states/claimed"
----
...
[source,sh]
----
 curl -X GET -H "Accept: application/json" --user <USER_ID>:<USER_PWD> "http://$HOSTNAME/kie-server/services/rest/server/queries/tasks/instances/owners"
----
...
[source,sh]
----
 curl -X PUT -H "Accept: application/json" --user <USER_ID>:<USER_PWD> "http://$HOSTNAME/kie-server/services/rest/server/containers/<CONTAINER_ID>/tasks/<TASK_ID>/states/started"
----
...
[source,sh]
----
 curl -X PUT -H "Accept: application/json" --user <USER_ID>:<USER_PWD> "http://$HOSTNAME/kie-server/services/rest/server/containers/<CONTAINER_ID>/tasks/<TASK_ID>/states/completed"
----
