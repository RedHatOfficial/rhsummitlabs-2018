...
curl -X POST --user <USER_ID>:'<USER_PWD>' \
  http://<HOST>:<PORT>/kie-server/services/rest/server/containers/$ALIAS/processes/org.jbpm.demo.NewApplication/instances \
  -H 'cache-control: no-cache' \
  -H 'accept: application/json' \
...
----
  ...
  * REQUEST:
  ...
  // TODO - Update this section around LDAP Groups and Users etc..
